...
if not settings.DEV_MODE:
    ...
    class serial:
        class Serial:
            ...
            def reset_input_buffer(self): pass # Dummy metod
            @property
...
class SensorManager:
    ...
    def _sniff_ports(self, duration_seconds: int = 15) -> Dict[str, str]:
        ...
        for port_name in potential_ports:
            ...
            except serial.SerialException as e: logger.warning(f"Could not open port {port_name} for sniffing: {e}")
    ...
    def discover_and_connect(self):
        # ... Bu metodda değişiklik yok ...
        logger.info("Discovering and connecting to sensors...")
        ...
        self._connect_all()
        logger.info("Sensor connection process finished.")
    ...
    def _connect_all(self):
        ...
        if self.height_port and not self.is_height_connected:
            try:
                ...
                logger.info(f"Height sensor connected on {self.height_port}")
        ...
        if self.weight_port and not self.is_weight_connected:
            try:
                ...
                logger.info(f"Weight sensor connected on {self.weight_port}")
        ...
        if not self.is_temp_hum_connected:
            try:
                ...
                logger.info(f"Temp/Humidity sensor connected on I2C bus {settings.I2C_BUS}")
            except (FileNotFoundError, PermissionError, OSError) as e:
                logger.error(f"Failed to connect to Temp/Humidity sensor (I2C): {e}")

    def prepare_for_reading(self):
        """Okuma döngüsünden önce seri port tamponlarını temizler."""
        if self.is_height_connected and self.height_ser:
            try:
                self.height_ser.reset_input_buffer()
                logger.debug("Height sensor input buffer cleared.")
            except Exception as e:
                logger.warning(f"Could not clear height sensor buffer: {e}")
        
        if self.is_weight_connected and self.weight_ser:
            try:
                self.weight_ser.reset_input_buffer()
                logger.debug("Weight sensor input buffer cleared.")
            except Exception as e:
                logger.warning(f"Could not clear weight sensor buffer: {e}")
